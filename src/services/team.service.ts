...
//     }
// }
export async function fetchTeamsCategByOrgId(
  orgId: string,
  userId: string,
  userRole: string
): Promise<any[]> {
  try {
    ...
    if (userRole !== "owner") {
      console.log(data[0].subcategories, "data");
      return data.map((team) => ({
        ...team,
        subcategories: team.subcategories.map((subcategory) => ({
          ...subcategory,
          knowledge_card: subcategory.knowledge_card.filter(
            (card) =>
              card.visibility === "everyone" ||
              (card.visibility === "only_me" &&
                card.card_owner_id?.id === userId) ||
              (card.visibility === "selected_users" &&
                card.knowledge_card_user_access?.some(
                  (a) => a.user_id === userId
                )) ||
              (card.visibility === "team_based" &&
                card.knowledge_card_user_access?.some(
                  (a) => a.user_id === userId
                ))
          ),
